...
if __name__ == "__main__":
    ...
    lines = parking.filter(lambda line: line != header) \
                       .map(lambda x: (x[1][:2]+x[1][5:],(1,0,0,0)) if x[11]=='FELONY' else (x[1][:2]+x[1][5:],(0,1,0,0)) if x[11]=='MISDEMEANOR' else (x[1][:2]+x[1][5:],(0,0,1,0)) if x[11]=='VIOLATION' else (x[1][:2]+x[1][5:],(0,0,0,1)) ) \
 		       .reduceByKey(lambda x,y: (x[0]+y[0], x[1]+y[1], x[2]+y[2], x[3]+y[3]))

    out = lines.map(lambda x: "%s\t%d, %d, %d, %d" %(x[0],x[1][0],x[1][1],x[1][2],x[1][3]))
    out.saveAsTextFile("year_CD.out")
    sc.stop()
